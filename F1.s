...
addi    a3, zero, 2147483648#Some number to initialise LFSR (2^31)
...
jalr    ra, secondTimer     #Jumps to the secondTimer subroutine to count 1s
jal     zero, -3            #Always jumps back to increment to next state

lastState:
jalr    ra, randTimer
addi    a0, zero, 0         #Return the state to 0
jal     zero, -8            #Always jump back outside the main loop to reset and wait for trigger again
...
addi    a2, a2, -1
bne     a2, zero, -1        #Keep looping back to -1 until 1s is reached
